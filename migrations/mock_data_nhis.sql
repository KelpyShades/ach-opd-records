-- Mock Data Generator for NHIS Records
-- This creates 5 records per month from Jan 2020 to Oct 2025
-- Total: ~350 records for demonstration purposes
-- Run this AFTER creating the records_nhis table
-- TO REMOVE: DELETE FROM records_nhis WHERE opd_number LIKE 'OPD-%';

-- Temporarily disable RLS for bulk insert
ALTER TABLE records_nhis DISABLE ROW LEVEL SECURITY;
...
BEGIN
    ...
    WHILE loop_date <= end_date LOOP
        ...
        FOR i IN 1..5 LOOP
            ...
            INSERT INTO records_nhis (opd_number, nhis_number, ccc, created_at)
            VALUES (
                'OPD-' || LPAD(record_counter::TEXT, 6, '0'),  -- OPD-000001, OPD-000002, etc.
                'NHIS' || LPAD((RANDOM() * 999999)::INT::TEXT, 6, '0'),  -- NHIS123456
                'CCC-' || CHR(65 + (RANDOM() * 25)::INT) || (RANDOM() * 999)::INT,  -- CCC-A123, CCC-B456
                mock_timestamp
            );
        END LOOP;
...
-- Re-enable RLS
ALTER TABLE records_nhis ENABLE ROW LEVEL SECURITY;
